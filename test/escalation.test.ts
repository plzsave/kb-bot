...
test("runWithEscalation: startHard=true は最初から上位ティアで実行する（A経路）", async () => {
  const provider = fakeProvider({ base: { text: "基本" }, hard: { text: "上位回答" } });
  const r = await runWithEscalation({
    provider, model: "base", modelHard: "hard", system: "s", messages: msg, tools: [], maxTurns: 5, startHard: true,
  });
  expect(r.result.text).toBe("上位回答");
  expect(r.escalated).toBe(true);
  expect(r.modelUsed).toBe("hard");
  expect(provider.models).toEqual(["hard"]); // base を一度も呼ばない
});

test("runWithEscalation: startHard=false + truncated で B 昇格する", async () => {
  const provider = fakeProvider({ base: "toolloop", hard: { text: "救済" } });
  const r = await runWithEscalation({
    provider, model: "base", modelHard: "hard", system: "s", messages: msg, tools: [], maxTurns: 3, startHard: false,
  });
...
test("runWithEscalation: modelHard 未設定なら truncated でも昇格しない（Req 3.5）", async () => {
  const provider = fakeProvider({ base: "toolloop" });
  const r = await runWithEscalation({
    provider, model: "base", modelHard: undefined, system: "s", messages: msg, tools: [], maxTurns: 2, startHard: false,
  });
...
test("runWithEscalation: 404/退役は既定モデルへフォールバックする", async () => {
  ...
  const r = await runWithEscalation({
    provider, model: "base", modelHard: undefined, system: "s", messages: msg, tools: [], maxTurns: 3, startHard: false,
  });
...
test("runWithEscalation: onEscalate は B経路再実行の直前に呼ばれる", async () => {
  ...
  await runWithEscalation({
    provider, model: "base", modelHard: "hard", system: "s", messages: msg, tools: [], maxTurns: 3, startHard: false,
    onEscalate: () => { calls.push("escalate"); },
  });
