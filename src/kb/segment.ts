...
const STOP = new Set([
  "は", "が", "を", "に", "へ", "と", "で", "も", "や", "の", "か", "ね", "よ", "さ",
  "し", "て", "た", "だ", "な", "する", "なる", "ある", "いる", "れる", "られる",
  "です", "ます", "ますか", "でしょ", "だろ", "ください", "下さい",
  "から", "まで", "より", "など", "けど", "ので", "のに", "って", "という",
  "これ", "それ", "あれ", "どれ", "この", "その", "あの", "どの", "こと", "もの",
  "について", "教え", "知り", "たい", "どう", "やっ", "ましょ",
]);

/** 索引用: 全トークンを空白連結（小文字化）。原文ではなくこの文字列を FTS の対象にする。 */
export function indexTokens(text: string): string {
  return seg.segment(text).map((t) => t.trim().toLowerCase()).filter(Boolean).join(" ");
}
