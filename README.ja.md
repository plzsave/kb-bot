...
1. **回答キャッシュ**（SQLite, 完全一致, 既定 TTL 24h）— ヒットすれば LLM を呼ばない＝最大の節約。
   ...
   キーには `provider:model` も含むので、`KB_LLM_PROVIDER` / `KB_MODEL` を切り替えても旧モデルの
   回答は配信されず、新モデルで再生成される
...
chmod 600 .env          # 秘密情報ファイル。所有者以外の読み取りを禁止
```

秘密情報の一覧、AI キーのワークスペース分離・支出上限設定、ローテーション手順は
[`docs/runbook-secrets.md`](docs/runbook-secrets.md) を参照。
...
2 つ同梱している。ナレッジを“ライフサイクルのあるもの”として扱う仕組みで、どちらも cron /
GitHub Actions / 定期コンテナで回す素の CLI だ。Bot 本体の中では動かないので、手書きドキュメント
だけで運用するなら無視してよい。

```bash
# 育てる: 解決済み GitHub issue を正規化 Markdown 化して取り込む
bun run kb:issues --repos owner/name        # 要約 .md を R2/S3 へ（増分）
bun run kb:ingest                            # 新規ドキュメントを FTS5 索引へ

# 退役: 陳腐化したナレッジにフラグ（既定はレポートのみ）
bun run kb:prune                             # 候補を報告（何も変更しない）
bun run kb:prune --apply                     # _stale/ へ隔離（可逆）→ 再取り込み
```

- **`kb:issues`（育てる）.** `KB_ISSUE_REPOS`（`KB_GITHUB_REPOS` とは分離）の *closed* issue を読み、
  LLM に要約させて frontmatter（`repo` / `closed_at` / `related_files`）付きの正規化 `.md` にし、R2/S3
  へ配置する。issue に埋もれた暗黙知が検索対象になる。**増分**（各 issue の `updated_at` を state
  ファイル `KB_ISSUE_STATE_FILE` で追跡）で、低シグナルな issue は除外（`KB_ISSUE_MIN_COMMENTS`）、
  隔離済み（_stale）は復活させない（tombstone）。`--dry-run` で確認、`--since` / `--max-issues` /
  `--model` で調整できる。
- **`kb:prune`（退役）.** R2/S3 のドキュメントを走査し、複数シグナルで陳腐化候補を立てる:
  **old**（`closed_at` が `--age-years` より古い）・**unused**（最近 retrieved 記録が無い＝各検索は
  どのドキュメントを引いたかを記録する）・**code_drift**（`related_files` が対象リポの HEAD に
  存在しない）。**削除はしない**: 既定はレポートのみで、`--apply` で候補を `_stale/`（検索対象外・
  戻せば復活）へ*移動*する。frontmatter の無い手書きドキュメントはスキップするので、機械生成の
  ナレッジしか触らない。
...
src/
  ...
  kb/
    ...
    ingest.ts      取り込みジョブ
    issueDoc.ts    issue → 正規化 Markdown（frontmatter・プロンプト）
    prune.ts       陳腐化フラグ判定（純関数・old / unused / code_drift）
  issues.ts        GitHub issue/コメント取得（kb:issues 用）
  usage.ts         retrieved 記録（kb:prune の unused を支える）
...
scripts/
  kb-ingest.ts / kb-search.ts   中核 CLI
  issue-to-kb.ts / kb-prune.ts  ナレッジのライフサイクル（育てる/退役）
  kb-eval.ts                    ルーティング eval（実 LLM＋GitHub）
